package com.wipro.evs.dao.impl;

import java.math.BigInteger;
import java.util.ArrayList;
import java.util.Date;
import org.hibernate.Query;
import org.hibernate.Session;
import org.hibernate.Transaction;
import org.hibernate.type.StandardBasicTypes;
import com.wipro.evs.bean.ElectionBean;
import com.wipro.evs.dao.ElectionDAO;
import com.wipro.evs.util.DBUtil;
import com.wipro.evs.util.impl.DBUtilImpl;

public class ElectionDAOImpl implements ElectionDAO {
	
	DBUtil db=new DBUtilImpl();
	ArrayList<ElectionBean> array;

	@Override
	public String createElection(ElectionBean ebean) {
		Session session = db.getDBConnection().openSession();
		Transaction tx = session.beginTransaction();
		Object obj=null;
		try{
			obj=session.save(ebean);
		}catch(Throwable e)
		{
			tx.rollback();
			return "ERROR";
		}
		if(obj!=null)
		{
			tx.commit();
			return "SUCCESS";
		}
		else
			return "FAIL";
	}

	@Override
	public int deleteElection(ArrayList<String> plist) {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public boolean updateElection(ElectionBean ebean) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public ElectionBean findByID(String pid) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public ArrayList<ElectionBean> findAll() {
		Session session = db.getDBConnection().openSession();
		Transaction tx = session.beginTransaction(); 
		array=(ArrayList<ElectionBean>) session.createCriteria(ElectionBean.class).list();
		return array;
	}

	@Override
	public ArrayList<ElectionBean> upcomingElection() {
		Session session = db.getDBConnection().openSession();
		Transaction tx = session.beginTransaction(); 
		Date date=new Date();
		java.sql.Date sqlDate = new java.sql.Date(date.getTime());
		Query query = session.createSQLQuery
		("select * FROM EVS_TBL_election WHERE electiondate > :sqldate" )
		.addEntity(ElectionBean.class);
		query.setDate("sqldate", sqlDate);
		array=(ArrayList<ElectionBean>) query.list();
		return array;
	}

	@Override
	public String getElectionSeq() {
		Session session = db.getDBConnection().openSession();
		Transaction tx = session.beginTransaction();
		Query query = session.createSQLQuery("select evs_seq_electionId.nextval as num from dual").addScalar("num",
				StandardBasicTypes.BIG_INTEGER);
		long id = ((BigInteger) query.uniqueResult()).longValue();
		return String.valueOf(id);
	}

}
